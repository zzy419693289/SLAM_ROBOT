...
void Communicating::Run()
{
	...
	Send_Count++;
	if(last_op_down || (Send_Count>30))	//if last instruction have been executed, then send next instruction. And a instruction most executed 30*100ms = 3s, then we will force to send next instrucion
	{
	    //write
	    Send_Count = 0;
