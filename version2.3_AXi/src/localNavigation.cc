...
vector<Send_Message> LocalNavigating::GoSendMesXi(vector<localGuide> LocalGuideSeq)
{
    ...
    This_Step.Data4 = 0x43; //0.158m/s,DogSpeed
    ...
    for(SeqCounter = 0;SeqCounter<LocalGuideSeq.size();SeqCounter++)
    {
		...
		float longdis = sqrt(pow(LocalGuideSeq[SeqCounter+1].n-LocalGuideSeq[SeqCounter].n, 2)+pow(LocalGuideSeq[SeqCounter+1].m-LocalGuideSeq[SeqCounter].m, 2));
		...
		if(outangle == 0)	//go straight
		{
			...
			int gotimes = longdis/DogSpeed;	//take same times instruction to go
...
/*    while(!our_Angle_Ptr->fisrtWayGet)	//wait until we have first angle
    ...
    {     
		...
		if(CountRestart>10)	//if 30s we do not move, then restart the system
		{
			CountRestart = 0;
			lastkfnum = 0;
			SetRestart();
