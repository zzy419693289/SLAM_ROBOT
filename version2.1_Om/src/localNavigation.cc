...
vector<Send_Message> LocalNavigating::GoSendMesXi(vector<localGuide> LocalGuideSeq)
{
    ...
    float LastWay = 0;
    for(SeqCounter = 0;SeqCounter+4<LocalGuideSeq.size();SeqCounter+=4)
    {
        float tangle = atan2(LocalGuideSeq[SeqCounter+4].n-LocalGuideSeq[SeqCounter].n, LocalGuideSeq[SeqCounter+4].m-LocalGuideSeq[SeqCounter].m)*180/3.14159;
