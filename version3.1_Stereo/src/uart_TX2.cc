...
void Communicating::Run()
{
	...
	if(last_op_down && (Send_Count>9))	//if last instruction have been executed, then send next instruction.if have yet no instruction, we will circle wait new instruction sequences arrived. make sure 2 instruction have 0.2 second delay
	{
	    //write
	    Send_Count = 0;
	...
	if(CheckFinish())
            break;
	usleep(20000);	//20ms check one message	
    }
