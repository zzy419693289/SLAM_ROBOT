...
public:
    ...
    LocalNavigating(double kfcx_, double kffx_, double kfcy_, double kffy_);
...
protected:  
    ...
    //plan a front way, when the delta of globalway and localway between at -60~60
    ...
    //plan a front way, without SLAM system
    std::vector<localGuide> PlanAFaceWayWithDepth(cv::Mat imD,float GlobalWay,float LocalWay);

    //calculate the real control sequence, A Xi
    vector<Send_Message> GoSendMesXi(vector<localGuide> LocalGuideSeq);

    //move backward instruction, A Xi
    vector<Send_Message> BackSendMesXi();
