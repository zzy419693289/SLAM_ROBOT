...
void GlobalNavigating::Trans_Baidu(GPS_data& GPS, float& Latitude, float& Longitude)
{
    Latitude = (GPS.N_latitude[0]-0x30)*10+(GPS.N_latitude[1]-0x30)+((GPS.N_latitude[2]-0x30)*10+(GPS.N_latitude[3]-0x30)+(GPS.N_latitude[5]-0x30)*0.1+(GPS.N_latitude[6]-0x30)*0.01+(GPS.N_latitude[7]-0x30)*0.001+(GPS.N_latitude[8]-0x30)*0.0001)/60;
    Longitude = (GPS.E_longitude[0]-0x30)*100+(GPS.E_longitude[1]-0x30)*10+(GPS.E_longitude[2]-0x30)+((GPS.E_longitude[3]-0x30)*10+(GPS.E_longitude[4]-0x30)+(GPS.E_longitude[6]-0x30)*0.1+(GPS.E_longitude[7]-0x30)*0.01+(GPS.E_longitude[8]-0x30)*0.001+(GPS.E_longitude[9]-0x30)*0.0001)/60;
}
...
void GlobalNavigating::Run()
{
	...
	{
		mpCommunicator->GPS_Export(mpCommunicator->our_GPS, our_GPS);
	}
	
	if (our_GPS.Effective)
	{	
	        Trans_Baidu(our_GPS, Now_Latitude, Now_Longitude);
