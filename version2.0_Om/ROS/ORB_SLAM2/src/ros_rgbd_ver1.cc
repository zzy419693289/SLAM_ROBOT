...
public:
    ...
    void GrabRGBD(const sensor_msgs::ImageConstPtr& msgRGB,const sensor_msgs::ImageConstPtr& msgD,const sensor_msgs::PointCloud2ConstPtr& msgPoint2);

    ORB_SLAM2::System* mpSLAM;
};

    //create global compare vector
    static vector<float> GlobalTrajectory;
    //create global octree
    static octomap::OcTree tree(0.1);
...
int main(int argc, char **argv)
{
    ...
    message_filters::Synchronizer<sync_pol> sync(sync_pol(20), rgb_sub,depth_sub,pointcloud_sub);	//sync
    // add the third parameter, and call back ImageGrabber, when get sync then callback
    sync.registerCallback(boost::bind(&ImageGrabber::GrabRGBD,&igb,_1,_2,_3));		//register callback
    ...
    //update the tree's occupancy
    tree.updateInnerOccupancy();

    // save the tree
    tree.writeBinary("mymap.bt");
    cout<<"map.bt is saved"<<endl;
...
//add cloudpoint2 msg
void ImageGrabber::GrabRGBD(const sensor_msgs::ImageConstPtr& msgRGB,const sensor_msgs::ImageConstPtr& msgD,const sensor_msgs::PointCloud2ConstPtr& msgPoint2)
{
	...
	tree.insertPointCloud(octomap_cloud, octomap::point3d(NewTrajectory[0], NewTrajectory[1], NewTrajectory[2]));
