...
void Communicating::GPS_Export(GPS_data* GPS1_ptr, GPS_data* GPS2_ptr)
{
	...
	GPS1_ptr->Effective = false;
}

bool Communicating::Check_Earthdata(char* buffer, Angle_data* our_Angle_ptr)
{
	if(buffer[2]==0xBB)	//stop byte
	{
	    unique_lock<mutex> lck(AngleMutex);
	    our_Angle_ptr->RobotNowWay=(buffer[0]-0x30)*256+buffer[1]-0x30;
...
void Communicating::Run()
{
	...
	if(Send_Count>19)	//50*20=1s
	{
		...
		std::cout << car_operate[2] << "   " << car_operate[3] << "   " << car_operate[4] << std::endl;
