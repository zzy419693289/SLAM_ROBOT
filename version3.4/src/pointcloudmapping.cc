...
pcl::PointCloud<PointCloudMapping::PointT>::Ptr PointCloudMapping::generatePointCloud(KeyFrame* kf, cv::Mat& color, cv::Mat& depth)
{
    ...
    for (int m=0; m<depth.rows; m+=FilterBand)	//the spare's point which have depth is standed for the point have reflectivity  
    {
        for (int n=0; n<depth.cols; n+=FilterBand)
        {
            float d = depth.ptr<float>(m)[n];
            if (d < 0.5 || d>5)		//indoor the farest is less than 3m
                continue;
            PointT p;
            p.z = d;
	    ...
	    float leftrightthrod = 1*p.z;	//we define the camera can only look 45'. Out of this threshold , drop the data

	    if (p.x < -1*leftrightthrod || p.x>leftrightthrod)     //pointcloud filter, left right boundary
                continue;	
            if (p.y < -0.5*leftrightthrod || p.y > 0.5*leftrightthrod || p.y > 1 || p.y < -1)	//high boundary,16:9, and robot lower than 1m
                continue;   
    ...
    {
	unique_lock<mutex> lck(poseMutex);
    	T_Pose = ORB_SLAM2::Converter::toSE3Quat( kf->GetPose() );	//from pose to get original transform
    }    
...
void PointCloudMapping::Run()
{
	...
	if(CheckFinish())
            break;
	usleep(5000);	//5ms a turn to make sure whether have the new frame
    }
