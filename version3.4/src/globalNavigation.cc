...
void GlobalNavigating::Run()
{
	...
	if(timeCount==8)
	{
		...
		if(tmpRecieved<3)	//we have no enough effectly GPS data
		{
			unique_lock<mutex> lck(GlobalWayMutex);
		...
		else	//GPS effect!
		{
			...
			for(int i=0;i<tmpRecieved;i++)
			{
				...
				if(tmp_Latitude[i]>max_La)
					max_La=tmp_Latitude[i];
				if(tmp_Latitude[i]<min_La)
					min_La=tmp_Latitude[i];
				if(tmp_Longitude[i]>max_Lo)
					max_Lo=tmp_Longitude[i];
				if(tmp_Longitude[i]<min_Lo)
					min_Lo=tmp_Longitude[i];
			}
			Now_Latitude=(Now_Latitude-max_La-min_La)/(tmpRecieved-2);
			Now_Longitude=(Now_Longitude-max_Lo-min_Lo)/(tmpRecieved-2);
